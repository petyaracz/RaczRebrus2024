...
```{r knn_res, fig.width = 6, fig.height = 4}
...
k %>% 
  ...
  theme_bw() +
  scale_x_continuous(sec.axis = sec_axis(trans = ~ plogis(.), breaks = c(.05,.25,.50,.75,.95), name = 'stem p(back)'), breaks = -2:2, name = 'stem log (back/front)') +
  scale_y_continuous(sec.axis = sec_axis(trans = ~ plogis(.), breaks = c(.05,.25,.50,.75,.95), name = 'knn p(back)'), breaks = -5:5, name = 'knn log (back/front)')
...
ggsave('viz/fig5.png', width = 8, height = 5)

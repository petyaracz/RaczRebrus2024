---
title: "SI. Lexical patterns in Hungarian derivational morphology"
...
n_ends = s2b %>%
  ...
  filter(ends_something) %>% 
  pull(n)

glm_hayes = glm(cbind(back,front) ~ 1 + ends_something, data = s2b, family = binomial)
hayes_est = broom::tidy(glm_hayes, conf.int = T) %>% 
  filter(term == 'ends_somethingTRUE') %>% 
  pull(estimate) %>% round(2)
hayes_low = broom::tidy(glm_hayes, conf.int = T) %>% 
  filter(term == 'ends_somethingTRUE') %>% 
  pull(conf.low) %>% round(2)
hayes_high = broom::tidy(glm_hayes, conf.int = T) %>% 
  filter(term == 'ends_somethingTRUE') %>% 
  pull(conf.high) %>% round(2)

```

If the stem ends in one of the categories from Hayes et al 2009, do we see the same pattern in our corpus data as they saw in their data (preference for front variants)? Yes. The paper argues that a stem is more likely to select a front vowel suffix if it ends in a labial stop, a sibilant, a coronal sonorant, or a consonant cluster. `r n_ends`/164 stems match one of these categories in our data. We fit a generalised linear model predicting the log odds of back suffixes if the stem matched any of the categories. We found that these stems are less likely to select a back suffix (est = `r hayes_est`, 95%CI: [`r hayes_low`;`r hayes_high`]).

## Visualisations

### Figure 1

```{r fig1, dpi = 500, fig.width = 10, fig.height = 6}
...
p1 = w %>% 
  ...
  mutate(
    ...
    `stem type` = case_when(
      back == 0 ~ 'disharmonic (front suffix only)',
      front == 0 ~ 'true neutral (back suffix only)',
      T ~ 'vacillating (both back / front suffixes)'
    )
         ) %>% 
...
```{r fig2, dpi = 500, fig.width = 7, fig.height = 12}

c %<>%
  mutate(
    example = stem %in% c('krapek','macesz','traverz')
  )

p3 = c %>% 
  filter(stem_varies_c,stem_varies_v) %>% 
  distinct(stem,example,v_minus_c,log_odds_back_c,log_odds_back_v) %>%
  # add nice names
  ...
  pivot_longer(-c(stem,example,stem2,v_minus_c,category)) %>% 
  ...
  ggplot(aes(name,value,label = stem2,group = stem2,colour = example)) +
  geom_line() +
...
p4 = c %>% 
  ...
  geom_label(aes(label = stem2,colour = example)) +
  geom_smooth() +
  theme_few() +
  geom_hline(yintercept = 0) +
  geom_vline(xintercept = c(qlogis(.01),qlogis(.99)), lty = 3) +
  ylab('front → back with linking V:\n(log (back/front) V + 10) -\n(log(back/front) C + 10)') +
...
```{r fig3, dpi = 500, fig.width = 6, fig.height = 5}
...
```{r fig4, dpi = 500, fig.width = 5, fig.height = 4}

sjPlot::plot_model(fit3b, 'pred', terms = c('knn', 'suffix_initial')) +
  theme_bw() +
...
```{r fig5, dpi = 500, fig.width = 6, fig.height = 4}
...
w %>% 
  ...
  getPLogOdds(stem) %>% 
  left_join(ety) %>%
  mutate(origin = fct_reorder(origin, n)) %>% 
  filter(!is.na(origin)) %>% 
...
```{r fig6, fig.width = 6, fig.height = 3}
...
pl_freq = suffix_frequencies[suffix_frequencies$suffix=='Pl',]$suffix_frequency
sum_freq = sum(suffix_frequencies$suffix_frequency)
sum_v_freq = sum(suffix_frequencies[suffix_frequencies$suffix_initial == 'V',]$suffix_frequency)
# pl_freq/sum_v_freq
# pl_freq/sum_freq
# sum_v_freq/sum_freq
```
...
p4 = c %>% 
  ...
  ylab('disharmonic → neutral with linking V:\n(log (back/front) V + 10) -\n(log(back/front) C + 10)') +
  scale_x_continuous(sec.axis = sec_axis(trans = ~ plogis(.), breaks = pbreaks, name = 'p (back)'), name = 'log (back / front):\ndisharmonic → neutral', breaks = c(-5:5)) +
...
k2 %>% 
  ...
  geom_hline(yintercept = c(qlogis(.01),qlogis(.99)), lty = 3) +
  scale_x_continuous(sec.axis = sec_axis(trans = ~ plogis(.), breaks = pbreaks, name = 'p (back)'), name = 'log (back/front):\ndisharmonic → neutral', breaks = c(-5:5)) +
  scale_y_continuous(sec.axis = sec_axis(trans = ~ plogis(.), breaks = pbreaks, name = 'KNN: p (back)'), name = 'KNN log (back/front):\ndisharmonic → neutral', breaks = c(-5:5))

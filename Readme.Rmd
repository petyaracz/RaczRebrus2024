...
`r c[c$stem_varies == T,]$n` out of `r nrow(c)` back vowel + \<e\> stems show back / front variation with at least one suffix. These stems have a bias towards front forms. This can be seen in Figure `r fcounter`.
...
The median of the distribution is p = `r round(plogis(unique(cq$median)),3)`. 50% of variable stems are either below p = `r round(plogis(unique(cq$lower)),3)` or above p = `r round(plogis(unique(cq$upper)),3)`.

Stem preference is determined by lexical stratum. A way to illustrate this is to look at the top and bottom 5 variable stems in the data, as seen in Table `r tcounter`.
...
c2 %>% 
  ...
  facet_wrap( ~ category) +
  xlab('') +
  scale_y_continuous(sec.axis = sec_axis(trans = ~ plogis(.), breaks = c(.01,.05,.1,.25,.5,.75,.9,.95,.1), name = 'random intercept (stem)\np (back)'), name = 'random intercept (stem)\nlog (back/front)', breaks = c(-5:5)) +
  ggtitle(glue('Figure {fcounter}. Back/front preference for variable stems across consonant-initial and vowel-initial suffixes'))

fcounter = fcounter + 1

```

The figure is split into two panels for the sake of legibility. The left panel shows stems that show a stronger back-preference with vowel-initial versus consonant-initial suffixes, the right panel, the other way round. In each panel, the two columns show the stem log odds with vowel-initial and consonant-initial suffixes only. Each line expresses the ratio of back preference with vowel- versus consonant-initial suffixes. For _haver_, seen on top in the left, this is very much a positive relationship. For _koncert_, seen at the bottom in the right, this is a weaker negative relationship.
...
c2 %>% 
  ...
  theme_bw() +
  geom_vline(xintercept = -4, alpha = .25) +
  scale_x_continuous(sec.axis = sec_axis(trans = ~ plogis(.), breaks = c(.01,.02,.05,.1,.25,.5,.75,.9,.95,.1), name = 'p (back)'), name = 'overall stem preference\nlog odds (back / front)', breaks = c(-10:5)) +
  ggtitle(glue('Figure {fcounter}. Overall stem preference and relative preference for variable stems.'))

cortest = c2 %>% 
  filter(log_odds_back > -4) %>% 
...
For any given stem, the vast majority of back / front variation in vowel-initial suffixes will likely come from the plural. For consonant-initial suffixes, this will be more evened out. This means that stems vary more across consonant-initial suffixes. 
...
We can plot the distribution of standard deviations for consonant-initial and vowel-initial suffixes across stems and suffixes that do show variation, using the formula `sqrt(n * p * (1 - p))` where n is the number of forms per stem and suffix type and p is the overall probability of back forms. This can be seen in Figure `fcounter`.
...
c2 %>% 
  ...
  scale_x_discrete(name = 'suffix type', labels = c('vowel-initial','consonant-initial')) +
...
c3 %>% 
  mutate(suffix_initial = fct_relevel(suffix_initial, 'V')) %>% 
...
modeltest = test_likelihoodratio(model_1,model_2) %>% 
  filter(Name == 'model_2')
chisq = pull(modeltest$Chi2)
```

This is indeed what we find: the more complex model provides a better fit (Chisq = `r round(chisq)`, p < .001, using a likelihood test).